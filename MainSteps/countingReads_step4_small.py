...
if strand == 'SECOND_READ_TRANSCRIPTION_STRAND':
    s_htseq = 'reverse'
elif strand == 'FIRST_READ_TRANSCRIPTION_STRAND':
    s_htseq = 'yes'
else:
    s_htseq = 'no'

def count(i):
    os.system('samtools view alignedReads/1pass_ann/' + i + 'Aligned.sortedByCoord.sortedByName.out.bam | htseq-count -a 10 -i ID -t miRNA -m union -s ' + s_htseq + ' - ' + gtfFile + ' > countedReads/1pass_ann/' + i + '.count')
